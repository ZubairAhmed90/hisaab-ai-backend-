/** 20 PSX stocks — every user sees these in the market list */
...
export type StockCatalogEntry = (typeof STOCK_CATALOG)[number];

export const CATALOG_BY_TICKER = Object.fromEntries(
  STOCK_CATALOG.map((s) => [s.ticker, s]),
) as Record<string, StockCatalogEntry>;

/** Realistic intraday tick — bounded random walk around base price */
export function simulateNextPrice(
  currentPrice: number,
  basePrice: number,
): { price_pkr: number; change_1d: number } {
...
export function catalogToMarketEntry(stock: StockCatalogEntry) {
  return {
    asset_type: 'stock',
    ticker: stock.ticker,
